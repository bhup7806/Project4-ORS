...
<%@page import="in.co.rays.project4.Ctl.CollegeListCtl" %>
...
%>
 ...
 <%
	...
	Iterator<CollegeBean> it = list1.iterator();
	if (list.size() != 0) {
...
<table>
      <tr>
					...
					<%= HTMLUtility.getList("name",String.valueOf(bean.getId()), list)%>&emsp;

						<label class="p1">City</label> <input type="text" name="city" placeholder="Entre City Name" class="p2"
						value="<%=ServletUtility.getParameter("city", request)%>">&emsp;&emsp;
						
						<label class="p1">State</label> <input type="text" name="state"placeholder="Entre State Name" class="p2"
...
<table width="100%">
	<tr>
	  <td><input type="submit" name="operation"  style="padding: 5px;" value="<%=TimeTableListCtl.OP_PREVIOUS%>"<%=pageNo > 1 ? "" : "disabled"%>></td>
	  <td><input type="submit" name="operation"  style="padding: 5px;" value="<%=TimeTableListCtl.OP_NEW%>">  </td>
	 <td><input type="submit" name="operation"  style="padding: 5px;" value="<%=TimeTableListCtl.OP_DELETE%>"> </td>
	 
	 <td align="right"><input type="submit" name="operation"  style="padding: 5px;" value="<%=TimeTableListCtl.OP_NEXT%>"<%=(nextPageSize != 0) ? "" : "disabled"%>> </td>
	  </tr>
	</table>
